...
class robot():
    ...
    def keeping(self,hsv):
        global LSD
        vel_msg=Twist()
        crop_L=hsv[400:480,0:220]
        crop_R=hsv[400:480,420:640]
...
#            time.sleep(1.2)
            vel_msg.linear.x = 0.01 #0.05
            vel_msg.angular.z = 0.0 # 0.6
            if yello_line2[0] is None:
                vel_msg.linear.x = 0.01 #0.05
                vel_msg.angular.z = 0.2 # 0.6

        elif white_line[0] is None: # and white_line2[0] is None:
#            time.sleep(1.2)
            vel_msg.linear.x = 0.01 #0.05
            vel_msg.angular.z = 0.0 # -0.6
            if white_line2[0] is None:
                vel_msg.linear.x = 0.01 #0.05
                vel_msg.angular.z = -0.2 # -0.6
        else :
